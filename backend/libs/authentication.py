...
def TokenRequired(f):
    ...
    def decorated(*args, **kwargs):
        ...
        token = data.get('jwt')
        if not token:
            return jsonify({'error': 'user token is missing'}), 403
        try:
            DecodeToken(token)
        except Exception:
            return jsonify({'error': 'user token is invalid/expired'}), 403
...
def CreateToken(username: str) -> str:
    ...
    if token is not None:
        ...
        if data and data.exp > datetime.datetime.utcnow(): return token
